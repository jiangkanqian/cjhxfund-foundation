...
public class StringTools {
	...
	/**
	 * 对URL进行编码，抄自java.net.URLEncoder，其不同之处在于空格不会转为+而是转为%20，
	 * 与前端js的encodeURIComponent效果保持一致
	 */
	...
	/**
	 * 对URL进行编码，抄自java.net.URLEncoder，其不同之处在于空格不会转为+而是转为%20，
	 * 与前端js的encodeURIComponent效果保持一致，使用UTF-8编码
	 */
	...
	/**
	 * 对URL进行解码，抄自java.net.URLDecoder，其不同之处在于+不会转为空格而是处理为%2B，
	 * 与前端js的decodeURIComponent效果保持一致
	 */
				...
				try {
					...
					if ((i < numChars) && (c == '%')) {
						throw new IllegalArgumentException(
								"decodeURIComponent: Incomplete trailing escape (%) pattern");
					}
					sb.append(new String(bytes, 0, pos, charset));
				} catch (NumberFormatException e) {
					throw new IllegalArgumentException(
							"decodeURIComponent: Illegal hex characters in escape (%) pattern - "
									+ e.getMessage());
				}
				needToChange = true;
				break;
	...
	/**
	 * 对URL进行解码，抄自java.net.URLDecoder，其不同之处在于+不会转为空格而是处理为%2B，
	 * 与前端js的decodeURIComponent效果保持一致，使用UTF-8解码
	 */
	...
	/**
	 * 生成格式化后的字符串，中文内容（仅限GB18030字符集中有的）一个字按2个ascii字符的宽度处理（jdk自带的String.
	 * format做不到这一点），如果包括异常参数，自动将其内容转化后输出
	 */
	public static String format(String format, Object... args) {
		if (null == args) {
			return format;
		}
		String fmt = new String(format.getBytes(Charsets.toCharset("GB18030")),
				Charsets.ISO_8859_1);
		...
		for (int i = 0; i < args.length; i++) {
			if (args[i] instanceof Throwable) {
				...
				objs[i] = new String(((String) args[i]).getBytes(Charsets
						.toCharset("GB18030")), Charsets.ISO_8859_1);
			} else {
				objs[i] = args[i];
			}
		}
		return new String(String.format(fmt, objs)
				.getBytes(Charsets.ISO_8859_1), Charsets.toCharset("GB18030"));
	}
		...
		if (args.length == sqls.length - 1) {
			...
			for (i = 0; i < args.length; i++) {
				...
				if (args[i] instanceof String) {
					...
					String src = args[i].toString().replace("'", "\\'")
							.replace("\r", "").replace("\n", "").trim();
					String append = src;
					if (src.length() > 120) {
						append = src.substring(0, 50)
								+ "...("
								+ args[i].toString().length()
								+ ")..."
								+ src.substring(src.length() - 50, src.length());
					}
					exsql.append("'").append(append).append("'");
				} else if (args[i] instanceof Date) { // Date统一转为Datetime处理
				// exsql.append("'").append(new Datetime(((Date)
				// args[i])).toString(Datetime.DF_yyyy_MM_dd_HHmmss_SSSZ)).append("'");
				} else if (args[i] instanceof byte[]) { // 二进制内容不显示，只显示长度信息
					exsql.append("[Binary Content ")
							.append(((byte[]) args[i]).length).append("]");
				} else {
					exsql.append(args[i]);
				}
			}
	 ...
	 * 
	 * @param tmp
	 *            存放结果的StringBuilder对象的引用
	 * @param len
	 *            字符的个数
	 * @param linechar
	 *            字符
	 */
	public static StringBuilder printLine(StringBuilder tmp, int len,
			char linechar) {
		for (int i = 0; i < len; i++) {
			tmp.append(linechar);
		}
	 ...
	 */
	private static StringBuilder _fixHexString(StringBuilder buf,
			String hexStr, int length) {
	...
	public static Boolean getBool(String value, Boolean defaultValue) {
		...
		if (value.equals("true") || value.equalsIgnoreCase("y")
				|| value.equals("1")) {
			return true;
		}
		return false;
	...
	private static final IllegalArgumentException classNotBasicTypeException = new IllegalArgumentException(
			"classNotBasicType");
	...
	private static Map<Type, Method> initValueOfMethod() {
		...
		try {
			valueOfMethods.put(int.class,
					Integer.class.getMethod("valueOf", String.class));
			valueOfMethods.put(Integer.class,
					Integer.class.getMethod("valueOf", String.class));
			valueOfMethods.put(long.class,
					Long.class.getMethod("valueOf", String.class));
			valueOfMethods.put(Long.class,
					Long.class.getMethod("valueOf", String.class));
			valueOfMethods.put(float.class,
					Float.class.getMethod("valueOf", String.class));
			valueOfMethods.put(Float.class,
					Float.class.getMethod("valueOf", String.class));
			valueOfMethods.put(double.class,
					Double.class.getMethod("valueOf", String.class));
			valueOfMethods.put(Double.class,
					Double.class.getMethod("valueOf", String.class));
			valueOfMethods.put(short.class,
					Short.class.getMethod("valueOf", String.class));
			valueOfMethods.put(Short.class,
					Short.class.getMethod("valueOf", String.class));
			valueOfMethods.put(boolean.class,
					Boolean.class.getMethod("valueOf", String.class));
			valueOfMethods.put(Boolean.class,
					Boolean.class.getMethod("valueOf", String.class));
			valueOfMethods.put(byte.class,
					Byte.class.getMethod("valueOf", String.class));
			valueOfMethods.put(Byte.class,
					Byte.class.getMethod("valueOf", String.class));
			valueOfMethods.put(String.class,
					String.class.getMethod("valueOf", Object.class));
			// 由于Character.valueOf方法的参数为char，对此需要使用一个特殊的方法
			valueOfMethods.put(char.class,
					StringTools.class.getMethod("getChar", String.class));
			valueOfMethods.put(Character.class,
					StringTools.class.getMethod("getChar", String.class));
		} catch (Exception e) {
	...
	@SuppressWarnings("unchecked")
	public static <T> T valueOf(String value, T defaultValue, Class<T> destClazz)
			throws Exception {
		if (isEmpty(value)) {
			return defaultValue;
		}
	...
	/**
	 * 将传入字符串的首字母小写，如果第2个字母也是大写首字母就不转为小写了（跟Spring的beanName规则保持一致，这也是java的get/
	 * set方法名生成规则）
	 */
	...
	/**
	 * 将传入字符串的首字母大写，如果第2个字母也是大写首字母就不转为大写了（跟Spring的beanName规则保持一致，这也是java的get/
	 * set方法名生成规则）
	 */
	...
	// try {
	// urlHref += "?" + e.getKey() + "=" +
	// URLEncoder.encode(String.valueOf(e.getValue()), Charsets.UTF_8.name());
	...
	public static String digestString(String src, int lengthThreshold) {
		if (src.length() > lengthThreshold * 2 + 20) {
			return src.substring(0, lengthThreshold)
					+ "...("
					+ src.length()
					+ ")..."
					+ src.substring(src.length() - lengthThreshold,
							src.length());
		}
		return src;
	}
