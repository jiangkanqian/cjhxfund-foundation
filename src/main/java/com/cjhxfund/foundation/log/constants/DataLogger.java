...
public class DataLogger extends JLoggerFactory {

	public DataLogger() {
	}

	public DataLogger(Class<?> clazz) {
		setClassName(clazz.getSimpleName());
		// logger.debug(className);
	}

	public static DataLogger getLogger(Class<?> clazz) {
		return new DataLogger(clazz);
	}
	...
	protected void writeLog(String logFileName, int level, String logMsg) {
		...
		FLogger.logManager.addLog(DataLogger.class.getSimpleName() + "-" + logFileName, sb);
